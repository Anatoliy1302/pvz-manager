/**
 * Статус доставки письма NotiSend (по id из логов send-auth-email).
 *
 *   node supabase/setup/check-notisend-delivery.mjs 20330591995
 *   node supabase/setup/check-notisend-delivery.mjs --send krv_kravec@mail.ru
 */
import fs from 'fs';
  ...
  switch (status) {
    ...
    case 'soft_bounced':
      return [
        ...
        '  1. NotiSend → Домены → pvzpersonal.ru → все DNS-записи зелёные (SPF, DKIM, DMARC).',
        '  2. Подождите 15–30 мин после правки DNS.',
...
if (!messageId) {
  console.error('Usage:');
  console.error('  node supabase/setup/check-notisend-delivery.mjs <message_id>');
  console.error('  node supabase/setup/check-notisend-delivery.mjs --send user@example.com');
  process.exit(1);
}
