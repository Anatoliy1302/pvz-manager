...
function loadPassword() {
  ...
  if (!match) throw new Error('SUPABASE_DB_PASSWORD missing in .env');
  return match[1].trim();
}
...
const sql = fs.readFileSync(
  path.resolve(process.cwd(), 'supabase/migrations/20250608160000_support_email_webhook.sql'),
  'utf8'
);
...
await mark.connect();
await mark.query(
  "insert into supabase_migrations.schema_migrations (version) values ('20250608160000') on conflict do nothing"
);
await mark.end();
console.log('Migration 20250608160000 (support email webhook) applied');
